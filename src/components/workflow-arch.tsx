...
/**
 * Workflow Architecture Diagram — Interactive Editor
 *
 * Arrow rendering: ALL arrows use the same rounded-corner L-shape path.
 * The only difference is the bendX parameter:
 *   - Solid arrows (dependsOn): bendX = midpoint between blocks
 *   - Dashed arrows (routes): bendX = right of all blocks
 */
...
const STATUS_COLORS: Record<string, string> = {
  pending: '#a1a1aa', waiting: '#f59e0b', running: '#3b82f6',
  completed: '#22c55e', failed: '#ef4444', blocked: '#9e9e9e', skipped: '#d4d4d4',
};
...
function getColor(action: string) {
  ...
  for (const [key, c] of Object.entries(ACTION_COLORS)) if (k.includes(key)) return c;
...
// ═══════ ARROW PATH ═══════
// Rounded-corner L-shape: source → horizontal → vertical → target
// bendX: where the vertical segment runs
// Returns SVG path string with Q-bezier rounded corners
...
function MiniMap({ steps, positions, zoom, pan, cs }: {
  steps: Step[]; positions: Map<string, { x: number; y: number; w: number; h: number }>;
  zoom: number; pan: { x: number; y: number }; cs: { w: number; h: number };
}) {
  const W = 140, H = 100;
  ...
  return (
    <div style={{ position: 'absolute', bottom: 12, right: 12, width: W, height: H, background: 'rgba(255,255,255,.92)', border: '1px solid #e4e4e7', borderRadius: 6, overflow: 'hidden', zIndex: 10 }}>
      <svg width={W} height={H}>
        {[...positions.entries()].map(([id, p]) => {
          const s = steps.find(s => s.id === id);
          const c = s ? getColor(s.action || 'execute') : ACTION_COLORS.execute;
          return <rect key={id} x={(p.x - mnX + 20) * sc + 5} y={(p.y - mnY + 20) * sc + 5}
            width={p.w * sc} height={p.h * sc} fill={c.fill} stroke={c.stroke} strokeWidth={.5} rx={1} />;
        })}
        <rect x={(-pan.x / zoom - mnX + 20) * sc + 5} y={(-pan.y / zoom - mnY + 20) * sc + 5}
          width={(cs.w / zoom) * sc} height={(cs.h / zoom) * sc}
...
// ═══════ CONTEXT MENU ═══════
function CtxMenu({ x, y, onClose, onEdit, onAdd, onDelete, onUp, onDown }: {
...
function Block({ step, x, y, w, h, run, onClick, onCtx, sel }: {
  ...
  run: Run | null; onClick?: () => void;
  onCtx?: (e: React.MouseEvent) => void; sel?: boolean;
}) {
  ...
  return (
    <g className="wf-block"
      ...
      onMouseEnter={e => {
        const r = e.currentTarget.querySelector('rect.block-main');
        if (r) { r.setAttribute('filter', 'url(#shadow)'); r.setAttribute('stroke', '#3b82f6'); r.setAttribute('stroke-width', '2'); }
      }}
      onMouseLeave={e => {
        const r = e.currentTarget.querySelector('rect.block-main');
        if (r) { r.removeAttribute('filter'); r.setAttribute('stroke', sel ? '#3b82f6' : sc); r.setAttribute('stroke-width', sel ? '2.5' : '1.5'); }
      }}>
      {/* Main rect */}
      <rect className="block-main" x={x} y={y} width={w} height={h}
        fill={isDone ? '#f0fdf4' : isFail ? '#fef2f2' : c.fill}
        stroke={sel ? '#3b82f6' : sc} strokeWidth={sel ? 2.5 : 1.5} rx={6}
      ...
      {isRun && step.agent && (
        <text x={x + w / 2} y={y + h / 2 + 10}
          fontSize={9} fontWeight={500} fill={c.text} opacity={.7}
          textAnchor="middle" dominantBaseline="middle"
          style={{ pointerEvents: 'none' }}>
          {step.agent}
        </text>
...
function Sidebar({ step, mode, onClose, onSave, onDelete }: {
  step: Step | null; mode: 'edit' | 'add'; onClose: () => void;
  onSave: (d: any) => void; onDelete?: () => void;
}) {
  const [id, setId] = useState(''), [ag, setAg] = useState(''), [act, setAct] = useState('execute');
  const [pr, setPr] = useState(''), [dep, setDep] = useState('');

  useEffect(() => {
    if (step && mode === 'edit') {
      setId(step.id); setAg(step.agent || ''); setAct(step.action || 'execute');
      setPr(step.prompt || ''); setDep((step.dependsOn || []).join(', '));
    } else if (mode === 'add') {
      setId(''); setAg(''); setAct('execute'); setPr(''); setDep(step ? step.id : '');
    }
  }, [step, mode]);

  if (!step && mode === 'edit') return null;

  const I: React.CSSProperties = { width: '100%', padding: '8px 10px', fontSize: 12, border: '1px solid #e4e4e7', borderRadius: 6, outline: 'none', boxSizing: 'border-box' as any, background: '#fff', color: '#18181b' };
  const B: React.CSSProperties = { padding: '8px 16px', fontSize: 12, fontWeight: 500, border: '1px solid #e4e4e7', borderRadius: 6, cursor: 'pointer' };
  const L: React.CSSProperties = { fontSize: 11, fontWeight: 500, color: '#71717a', marginBottom: 4, display: 'block' };

  return (
    <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 320, background: '#fff',
      borderLeft: '1px solid #e4e4e7', boxShadow: '-4px 0 16px rgba(0,0,0,.08)',
      display: 'flex', flexDirection: 'column', zIndex: 20, animation: 'wf-slide-in .2s ease-out' }}>
      ...
      <div style={{ flex: 1, padding: 20, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div><label style={L}>Action</label><select value={act} onChange={e => setAct(e.target.value)} style={I}>
          {['create', 'review', 'fix', 'verify', 'deploy', 'research', 'execute'].map(a => <option key={a} value={a}>{a}</option>)}
        </select></div>
      ...
      <div style={{ padding: '16px 20px', borderTop: '1px solid #e4e4e7', display: 'flex', gap: 8 }}>
        ...
        <button onClick={() => onSave({ id, agent: ag, action: act, prompt: pr, dependsOn: dep.split(',').map(s => s.trim()).filter(Boolean) })}
...
export default function WorkflowArch({ steps, run, onStepClick, onStepAdd, onStepDelete, onEdgeClick }: Props) {
  ...
  const onPM = useCallback((e: React.PointerEvent) => {
    ...
    setPan({ x: e.clientX - panSt.current.x, y: e.clientY - panSt.current.y });
  }, []);
  ...
  const onWh = useCallback((e: React.WheelEvent) => {
    ...
    setZoom(p => {
      const nz = Math.min(3, Math.max(.3, p + (e.deltaY > 0 ? -.1 : .1)));
      const s = nz / p;
  ...
  // Get effective position (base + drag offset)
  const gp = useCallback((id: string) => {
    const b = positions.get(id);
    if (!b) return null;
    return b; // No drag offset for now
  }, [positions]);

  // Right-click context menu position
  const getCtxPos = useCallback((e: React.MouseEvent) => {
  ...
  return (
    <div ref={cRef}
      ...
      <svg ref={sRef} width="100%" viewBox={`0 0 ${SVG_W} ${svgH}`}
        ...
        {/* ── Arrows (dependsOn) — rounded-corner L-shape ── */}
        ...
        {/* ── Route arrows (conditional) — same rounded-corner L-shape, bendX = right of all blocks ── */}
        {(() => {
          let maxRight = 0;
        ...
        {[...positions.entries()].map(([sid, p]) => {
          ...
          return (
            <Block key={sid} step={step} x={p.x} y={p.y} w={p.w} h={p.h}
              ...
              onCtx={e => {
                const pos = getCtxPos(e);
                setCtx({ x: pos.x, y: pos.y, step });
              }}
            />
          );
        })}

        {/* ── Context menu (inside SVG, positioned absolutely) ── */}
        {ctx && (
          <foreignObject x={ctx.x} y={ctx.y} width={150} height={200} style={{ overflow: 'visible' } as any}>
            <CtxMenu x={0} y={0} onClose={() => setCtx(null)}
              ...
              onUp={() => setCtx(null)} onDown={() => setCtx(null)} />
          </foreignObject>
        )}
      </svg>

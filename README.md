...
Mind Agency 给你一个 AI 团队：你定义角色，AI 按角色分工。有分歧？投票决定。需要你批准？自动暂停等你。
...
每个 Agent 有自己的角色、专长和行为风格。你可以创建任意数量的 Agent，给它们不同的职责。它们通过群聊和邮件协作，就像真正的团队。
...
**你可以创建更多：**
```
Agent: Diana — 前端专家，专注 React / UI
Agent: Eve — 安全审计，专注漏洞扫描
Agent: Frank — 文档工程师，自动生成 API 文档
```

每个 Agent 有独立的配置、记忆和行为画像。你可以让它们用不同的模型、不同的风格、不同的专长。

```
你: @Diana 帮我写一个用户注册接口
Diana: 好的，我来写 React 组件
Diana: @Eve 代码写好了，帮我做安全审查
Eve: 发现两个问题：1. 缺少输入校验 2. 密码没有加密
Diana: 改好了，你再看看
Eve: ✅ 安全通过
Diana: @Charlie 帮我跑一下测试
Charlie: 测试全部通过 ✅
```
...
```
Diana 写前端 → Eve 安全审查 → Charlie 测试 → [等你批准] → 部署
```
...
2. **创建 Agent** — 给每个 Agent 起名字、设角色、写行为描述
3. **建立群组** — 把 Agent 拉进群组，定义工作流
4. **开始协作** — 给 Agent 分配任务，看它们自动协作

系统自带 Alice / Bob / Charlie 三个示例 Agent，开箱即用。你也可以完全自定义。
...
你可以创建任意数量的 Agent，每个都有独立的角色、行为和记忆。
...
- **行为风格** — 严谨 / 冒险 / 务实
- **关注领域** — 安全 / 性能 / 用户体验
- **避免话题** — 某些不想让 Agent 碰的领域
- **记忆** — 跨会话的经验积累

Agent 数量没有上限，取决于你的 API 预算和硬件。

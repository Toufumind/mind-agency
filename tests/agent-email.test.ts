...
import { loadAgentEmails, sendAgentEmail } from '../src/lib/agent-email';

describe('Agent Email', () => {
  it('should load empty emails for new agent', async () => {
    const emails = await loadAgentEmails('test-email-new');
    expect(Array.isArray(emails)).toBe(true);
  });

  it('should send email', async () => {
    const result = await sendAgentEmail('test-email-send', 'bob', 'Test Subject', 'Test body');
